...
import { Upload, FileText, X, Loader2, CheckCircle, AlertCircle, Sparkles, Image, Languages, Globe, FileSearch, Activity } from 'lucide-react';
...
export default function UploadReportPage() {
    ...
    const handleLanguageChange = async (lang: Language) => {
        ...
        if (result && result.success && result.data?.analysis) {
            ...
            setLoading(true);
            try {
                const response = await fetch(`${API_BASE_URL}/api/reports/demo-translate?language=${lang}`, {
                    method: 'POST',
                    headers: {
    ...
    const handleAnalyze = async () => {
        ...
        try {
            ...
            if (selectedLanguage === 'english') {
                endpoint = '/api/reports/demo-upload';

                const fullUrl = `${API_BASE_URL}${endpoint}`;
                console.log('[Upload-Report] Fetching from:', fullUrl);
                ...
                console.log('[Upload-Report] Response status:', response.status, response.statusText);
                ...
                endpoint = `/api/reports/demo-upload-and-translate?language=${selectedLanguage}`;
            ...
            <div className="max-w-4xl mx-auto">
                ...
                <div className="text-center mb-8">
                    ...
                    <p className="text-slate-600 dark:text-slate-400">
                        Upload your lab report for instant AI-powered analysis and simplification
                    </p>
                </div>
                ...
                <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-xl border border-slate-200 dark:border-slate-800 p-8">
                    {!file ? (
                        <div
                            ...
                            className="border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-2xl p-12 text-center cursor-pointer hover:border-blue-500 dark:hover:border-blue-500 transition-colors"
                        >
                            <input
                                ref={fileInputRef}
                                type="file"
                                ...
                                className="hidden"
                            />
                            <div className="w-16 h-16 mx-auto mb-4 rounded-full bg-blue-100 dark:bg-blue-900/30 text-blue-600 flex items-center justify-center">
                                <FileSearch size={32} />
                            </div>
                                ...
                                <div className="flex-1">
                                    ...
                                    {!result && (
                                        <button
                                            ...
                                            disabled={loading}
                                            className="w-full py-4 rounded-xl font-bold flex items-center justify-center gap-2 transition-all bg-blue-500 hover:bg-blue-400 text-white disabled:opacity-50"
                                        >
                                            {loading ? (
                                                <>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md transition-shadow">
                            <div className="flex items-center gap-4 mb-4">
                                ...
                                <h3 className="font-bold text-lg text-slate-900 dark:text-white">Health Insights</h3>
                            </div>
                            <p className="text-slate-500 dark:text-slate-400 leading-relaxed">
                                Get meaningful insights from your lab values with risk assessment and recommendations
                            </p>
                        </div>
                        <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md transition-shadow">
                            <div className="flex items-center gap-4 mb-4">
                            ...
                            <p className="text-slate-500 dark:text-slate-400 leading-relaxed">
                                Get your report explained in Hindi & Marathi for easy understanding by everyone
                            </p>
                        </div>
                        <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md transition-shadow">
                            <div className="flex items-center gap-4 mb-4">
                                <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-green-400 to-green-600 text-white flex items-center justify-center shadow-lg shadow-green-500/20">
                                    <CheckCircle size={24} />
                                </div>
                                <h3 className="font-bold text-lg text-slate-900 dark:text-white">Plain English</h3>
                            </div>
                            <p className="text-slate-500 dark:text-slate-400 leading-relaxed">
                                Complex medical values explained in simple terms with actionable lifestyle tips
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div className="text-center mt-8">
                    <a href="/dashboard" className="text-slate-500 hover:text-blue-600 transition-colors">
                        ← Back to Dashboard
                    </a>
                </div>
            </div>
